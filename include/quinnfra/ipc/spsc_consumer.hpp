...
/// @brief Consumer view of the lock-free shared memory SPSC queue.
...
public:
    ...
    explicit SpscConsumer(Layout* layout) noexcept
        : layout_{layout},
          cached_write_index_{layout ? layout->write_index.load(std::memory_order_relaxed) : 0} {}

    /// @brief Attaches an RAII-managed consumer to an existing queue in shared memory.
    static std::optional<SpscConsumer<T, Capacity>> attach(std::string_view shm_name) {
        auto mapping = detail::RingBufferMapping<Layout>::attach(shm_name);
        if (!mapping.has_value()) {
            return std::nullopt;
        }

        return mapping ? std::optional<SpscConsumer<T, Capacity>>(SpscConsumer(std::move(*mapping)))
                       : std::nullopt;
    }
...
private:
    explicit SpscConsumer(detail::RingBufferMapping<Layout>&& mapping) noexcept
