...
/// @brief Producer view of the lock-free shared memory SPSC queue.
...
public:
    ...
    explicit SpscProducer(Layout* layout) noexcept
        : layout_{layout},
          cached_read_index_{layout ? layout->read_index.load(std::memory_order_relaxed) : 0} {}

    /// @brief Creates an RAII-managed producer that owns and initializes the queue in shared memory.
    static std::optional<SpscProducer<T, Capacity>> create(std::string_view shm_name) {
        auto mapping = detail::RingBufferMapping<Layout>::create(shm_name);
        return mapping ? std::optional<SpscProducer<T, Capacity>>(SpscProducer(std::move(*mapping)))
                       : std::nullopt;
    }
...
private:
    explicit SpscProducer(detail::RingBufferMapping<Layout>&& mapping) noexcept
